//! MKHS: Multi-Key Homomorphic Signatures over BLS12-381 for vector messages.
//!
//! This module implements the protocol described in "Context-Hiding Multi-Key Linearly
//! Homomorphic Authenticators" (Schabhüser et al.).
//!
//! - Each client gets a key pair and can sign a vector of field elements.
//! - Signatures and certain components are additively aggregatable across clients and rows.
//! - The aggregator can evaluate (aggregate) signatures and a verifier can check correctness against public keys and messages.
...
/// Internal per-client component embedded in signatures (includes Ed25519 tag and group elements).
pub struct Lam {
    client_id: u64,
...
#[derive(Debug, Clone)]
/// Aggregated signature across one or more clients for a vector message.
...
#[derive(Debug, Clone)]
/// Secret key material for MKHS (also includes an Ed25519 signing key for tagging).
pub struct SK {
    k: u64,
    sk_sig: SigningKey,
    xs: Vec<Fr>,
...
#[derive(Debug, Clone)]
/// Public key for MKHS, including Ed25519 verifying key and pairing-based helpers.
pub struct PK {
    pk_sig: VerifyingKey,
...
#[derive(Debug, Clone)]
/// Convenience container for MKHS secret and public keys.
pub struct KeyPair {
    pub sk: SK,
    pub pk: PK,
}
...
#[derive(Debug, Clone)]
/// MKHS system parameters and helpers.
pub struct Mkhs {
    g1: G1Projective,
    g2: G2Projective,
...
impl Mkhs {
    /// Setup MKHS parameters for `n` clients and vector length `t`.
    ...
    /// Generate a key pair for a specific `client_id`.
    ...
    /// Sign a vector message with the MKHS secret key.
    ...
    /// Aggregate multiple signatures into a single signature (component-wise addition).
    ...
    /// Verify a (possibly aggregated) signature against public keys and the aggregated message.
    pub fn verify(
        &self,
        pks: &HashMap<u64, PK>,
        messages: &[Fr],
